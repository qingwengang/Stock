...
public class StockTransactionDetailDao {
	...
	public static List<StockTransactionDetail> Query(String sql){
		...
		Session session=sf.openSession();
		session.beginTransaction();
		l=session.createSQLQuery(sql).addEntity(StockTransactionDetail.class).list();
		for (Object o : l) {
	...
	public static StockTransactionDetail QueryUnique(String sql){
		...
		Session session=sf.openSession();
		session.beginTransaction();
		o=session.createSQLQuery(sql).addEntity(StockTransactionDetail.class).uniqueResult();
		session.getTransaction().commit();
		session.close();
